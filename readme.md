...
2. Refresh/keepalive token nếu token sắp hết hạn.
3. Kiểm tra ngày hiện tại có nằm trong `schedule.days` hay không.
4. Nếu ngày bị khai báo trong `skipDates`, chỉ keepalive token rồi skip report.
5. Nếu chưa tới `parentPostAfterTime`, skip.
6. Nếu tới `parentPostAfterTime` nhưng chưa tới giờ report, chỉ tìm hoặc tạo post cha.
7. Nếu tới giờ report:
